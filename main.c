...
//	int reading;   
	...
	while(1)
	{
		...
		error = reading;
		error = (error & 0x1800) >> 11;
		reading = reading & 0x07FF; 
		//if ((reading >= 0) //&& (reading < 2047))
		//{
			distance = reading;	
		//}
		_delay_ms(100);
...
void oliviaSpecCfg( void ) 
{
	...
	i2c_write_16(oliviaAddress, 0x24, 0xF040);  //addr_ADDA_HFCFG2
...
int oliviaMeasurementOneSample( void ) 
{
	...
	if(successfulMeasurement)
	{
		...
		int measurement = (output & 0x7FFC) >> 2; // keep only 11bit data and 2bit error sign
		return measurement;
	} else {
